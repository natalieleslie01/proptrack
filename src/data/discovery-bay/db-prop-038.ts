import { Property } from '@/app/property-management/components/mockData';

const property: Property = {
  id: 'bij-00031',
  unit: '31 Bijou Hamlet',
  building: 'Bijou Hamlet',
  shortCode: 'BIJ00031',
  district: 'Discovery Bay, Lantau Island',
  street: 'Bijou Hamlet',
  type: 'Residential',
  status: 0,
  occupancyStatus: 'vacant',
  contactStatus: 'active',
  bedrooms: 3,
  bathrooms: null,
  sqft: 1556,
  floor: '',
  yearBuilt: 1994,
  monthlyRent: 80000,
  salePrice: null,
  landlord: {
    name: '',
    phone: '',
    email: '',
    idNumber: '',
  },
  tenant: null,
  lastUpdated: '20/05/2024',
  updatedBy: '',
  agentNotes: 'Lease: 80K @HK$51/sqft. Keys: Other (call II).',
  highlight: 'No golf cart no club membership',
  photos: ['/assets/images/no_image.png'],
  hasFloorPlan: false,
  keyLocation: { type: 'agent', agentName: 'call II' },
};

export default property;
