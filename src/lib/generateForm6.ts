...
export async function generateForm6PDF(data: Form6Data): Promise<void> {
  ...
  doc.setFontSize(13);
  doc.setFont('helvetica', 'bold');
  doc.text('FORM 6', pageW / 2, y, { align: 'center' });
  y += 6;
  doc.setFontSize(10);
  doc.text('PROPERTY INFORMATION FORM FOR LEASING', pageW / 2, y, { align: 'center', maxWidth: contentW });
  y += 5;
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'bold');
  doc.text('1.  PROPERTY INFORMATION', margin + 2, y + 4);
  doc.setFont('helvetica', 'normal');
  y += 9;
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(data.propertyAddress || '_______________________________________________', margin + 36, y, { maxWidth: contentW - 36 });
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Year Built:', margin, y);
  doc.setFont('helvetica', 'normal');
  doc.text(data.yearBuilt || '____', margin + 20, y);
  y += 7;
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Rent Includes:', margin, y);
  doc.setFont('helvetica', 'normal');
  tick(doc, margin + 28, y - 3.5, data.includesRates);
  doc.text('Government Rates', margin + 34, y);
  tick(doc, margin + 80, y - 3.5, data.includesManagement);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Pets Allowed:', margin, y);
  doc.setFont('helvetica', 'normal');
  tick(doc, margin + 28, y - 3.5, data.petAllowed);
  doc.text('Yes', margin + 34, y);
  tick(doc, margin + 50, y - 3.5, !data.petAllowed);
  doc.text('No', margin + 56, y);
  y += 10;
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(data.landlordBankDetails || '_______________________________________________', margin + 26, y, { maxWidth: contentW - 26 });
  y += 10;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('3.  DEPOSIT & PAYMENT', margin + 2, y + 4);
  doc.setFont('helvetica', 'normal');
  y += 9;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('4.  ESTATE AGENT DETAILS', margin + 2, y + 4);
  doc.setFont('helvetica', 'normal');
  y += 9;
  ...
  doc.save(`Form6_${data.landlordName || 'Landlord'}_${data.propertyAddress || 'Property'}.pdf`);
}
